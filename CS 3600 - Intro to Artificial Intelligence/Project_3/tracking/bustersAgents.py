...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        import operator
        bestPositions = [max(distribution.iteritems(), key = operator.itemgetter(1))[0]
                         for distribution
                         in distributions]
        bestPosition = min(bestPositions, key = lambda pos: self.distancer.getDistance(pos, pacmanPosition))
        newPositions = {action : self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), bestPosition)
                        for action
                        in legal}
        return min(newPositions.iteritems(), key = operator.itemgetter(1))[0]
