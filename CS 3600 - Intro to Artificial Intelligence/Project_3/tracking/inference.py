    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                overallDist[newPos] += prob * self.beliefs[oldPos]
        overallDist.normalize()
        """
        for pos in self.legalPositions:
            self.beliefs[pos] *= overallDist[pos]
            """
        self.beliefs = overallDist#normalize()
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for part in self.particles:
                ...
                beliefs[part] += emissionModel[trueDistance]
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for ghost in range(self.numGhosts):
            while weights[ghost].totalCount() == 0:
